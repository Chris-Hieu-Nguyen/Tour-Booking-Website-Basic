...
<div class="container-fluid">
	<h1 class="h3 mb-2 text-gray-800">Edit Tour</h1>
	{{Form::open(['url' => route('tour.update',['id'=>$tour->id])])}}
	<div class="row">
		<div class="col-9">
			<nav>
			...
			<div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
				<div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
					<div class="form-group">
						...
						{{ Form::text('title[vi]',$tour->translation('vi')->title,['class' => 'form-control input-border' ])}}
						{!! $errors->has('title.vi') ? '<p class="text-danger">'.$errors->first('title.vi').'</p>' : '' !!}
					</div>
					<div class="form-group">
						<label class="text-dark">Mô Tả Ngắn</label>
						{{Form::textarea('brief[vi]',$tour->translation('vi')->brief,['class'=>'form-control input-border','rows' => '2', 'cols'=>'30'])}}
						{!! $errors->has('brief.vi') ? '<p class="text-danger">'.$errors->first('brief.vi').'</p>' : '' !!}
					</div>
					<div class="form-group">
						<label class="text-dark">Mô Tả Chi Tiết</label>
						{{ Form::textarea('content[vi]',$tour->translation('vi')->description,['class'=>'form-control','id'=>'content_vi'])}}
				...
				<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
					<div class="form-group">
                        ...
                        {{ Form::text('title[en]',
                            $tour->translation('en')->title ? $tour->translation('en')->title : "",
                            ['class' => 'form-control input-border' ])}}
					</div>
					<div class="form-group">
						<label class="text-dark">Short description</label>
                        {{Form::textarea('brief[en]', 
                            $tour->translation('en')->brief ? $tour->translation('en')->brief : "",
                            ['class'=>'form-control input-border','rows' => '5', 'cols'=>'30'])}}
					</div>
					<div class="form-group">
						<label class="text-dark">Description</label>
                        {{ Form::textarea('content[en]',
                            $tour->translation('en')->description ? $tour->translation('en')->description : "",
                            ['class'=>'form-control','id'=>'content_en'])}}
					</div>					
				</div>
			</div>
			<div class="form-row mb-1">
				...
				<select name="destination_id" id="" class="form-control input-border">
						@foreach ($destination as $item)
							@if($item->id==$tour->destination_id)
				...
				<div class="col">
					...
					{{Form::number('price',(int) $tour->price,['class' => 'form-control input-border'])}}
				</div>
				<div class="col">
					<label class="text-dark">Giá (trẻ em >8t)</label>
					{{Form::number('price_child',(int) $tour->price_child,['class' => 'form-control input-border'])}}
				</div>
			</div>
			<div class="form-row mb-3">
				<div class="col">
					<label class="text-dark"> Khuyến Mãi(%)</label>
					{{Form::number('sale',$tour->sale,['class' => 'form-control input-border','min' => '0'])}}
				</div>
				<div class="col">
					<label class="text-dark"> Số Người</label>
					{{Form::number('max_people',$tour->max_people,['class' => 'form-control input-border','min' => '0'])}}
				</div>
				<div class="col">
					<label class="text-dark">Bắt đầu</label>
					{{Form::text('start_day',$tour->start_day,['class' => 'form-control input-border','placeholder' => 'mm/dd/yy','id' => 'from'])}}
					{!! $errors->has('start_day') ? '<p class="text-danger">'.$errors->first('end_day').'</p>' : '' !!}
				</div>
				<div class="col">
					<label class="text-dark">Kết thúc</label>
					{{Form::text('end_day',$tour->end_day,['class' => 'form-control input-border','placeholder' => 'mm/dd/yy','id' => 'to'])}}
		...
		<div class="col-3">
			<div class="form-group">
				<button class="btn bg-blue form-control text-white" style="border-radius:unset;"><i class="fab fa-telegram-plane"></i> &nbsp Edit Tour</button>
			</div>
			<div class="form-group">
				<label for="photo" class="">{{__('label.avatar')}}</label>
				{!! Form::hidden('avatar', '', ['id'=>'photo']) !!}
            <a href="javascript:openCustomRoxy()"><img src='{{$tour->avatar ? $tour->avatar : "/img/no-photo.png"}}' id="customRoxyImage" style="max-width:250px;"></a>
